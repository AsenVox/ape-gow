...
import jokerCard from '../assets/cards/JOKER.png'

// Import all provided card PNGs (2-9 + T/J/Q/K/A + joker)
const cardPngs = import.meta.glob('../assets/cards/*.png', { eager: true, import: 'default' }) as Record<
  string,
  string
>

const cardPngByName: Record<string, string> = Object.fromEntries(
  Object.entries(cardPngs).map(([p, url]) => {
    const name = p.split('/').pop() || p
    return [name.toUpperCase(), url]
  }),
)
...
export function CardFace({ card, onClick, tone = 'neutral', faceDown = false, title }: Props) {
  ...
  const faceSrc = !isJoker ? cardPngByName[cardImageName(card.rank, card.suit)] : undefined
